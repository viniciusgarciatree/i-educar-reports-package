...
class RegistrationCertificateReport extends Portabilis_Report_ReportCore
{
    use JsonDataSource;
...
SELECT fcn_upper(instituicao.nm_instituicao) AS nm_instituicao,
       ...
       (SELECT turma.nm_turma
		 ...
		 WHERE turma.cod_turma = matricula_turma.ref_cod_turma and
		       matricula_turma.ref_cod_matricula = matricula.cod_matricula AND matricula_turma.ativo = 1 order by matricula_turma.data_cadastro desc limit 1) as nm_turma,
  ...
  pessoa_pai.idpes = fisica.idpes_pai), aluno.nm_pai, 'NAO INFORMADO')) as nm_pai,

  fcn_upper(COALESCE((select pessoa_mae.nome from cadastro.pessoa as pessoa_mae where
  pessoa_mae.idpes = fisica.idpes_mae), aluno.nm_mae, 'NAO INFORMADO')) as nm_mae,
