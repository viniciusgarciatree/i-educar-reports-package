...
class ConclusionCertificateReport extends Portabilis_Report_ReportCore
{
  ...
  pessoa_mae.idpes = fisica.idpes_mae), aluno.nm_mae, '')) as nm_mae,
      ...
      (SELECT public.fcn_upper(substring(logradouro.nome from 1 for 1)) ||
              lower(substring(logradouro.nome, 2))
     ...
     (SELECT COALESCE((SELECT COALESCE((SELECT to_char(endereco_pessoa.cep::integer, '99999-999')
        ...
        WHERE juridica.idpes = endereco_pessoa.idpes AND
              juridica.idpes = escola.ref_idpes),
        (SELECT to_char(endereco_externo.cep::integer,'99999-999') FROM cadastro.endereco_externo WHERE endereco_externo.idpes = escola.ref_idpes))),
