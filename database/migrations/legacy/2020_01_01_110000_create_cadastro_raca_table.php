...
class CreateCadastroRacaTable extends Migration
{
    ...
    public function up()
    {
        if((DB::select("select EXISTS (SELECT FROM pg_catalog.pg_tables WHERE schemaname = 'cadastro' AND tablename = 'raca');"))[0]->exists == false) {
            DB::unprepared(
                '
            SET default_with_oids = true;
            
            CREATE SEQUENCE IF NOT EXISTS cadastro.raca_cod_raca_seq
                START WITH 1
                INCREMENT BY 1
                MINVALUE 0
                NO MAXVALUE
            ...
            CREATE TABLE IF NOT EXISTS TABLE cadastro.raca (
